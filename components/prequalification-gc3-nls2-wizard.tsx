...
export function PrequalificationGC3NLS2Wizard() {
  ...
  const getTotalSteps = () => {
    let totalSteps = 1 // Start with lease question
  ...
  const getLastStepIndex = () => {
    ...
    if (isOnLease === true) {
      lastStep = 1 // Lease type

      if (leaseType === "12-month") {
        lastStep = 2 // Lease expiry

        if (leaseExpiryDate) {
          if (isWithin60Days(leaseExpiryDate)) {
            lastStep = 3 // Notice to vacate

            if (hasNoticeToVacate === "submitted") {
              lastStep = 5 // After notice accepted
            } else {
              lastStep = 6 // Earliest move date
            }
          } else {
            lastStep = 4 // Notice to vacate for non-expiring lease
          }
        }
      } else if (leaseType === "month-to-month") {
        lastStep = 6 // After payment day
      }
    }
  ...
  const handleFinish = () => {
    ...
    localStorage.setItem("userRoleCode", "GC3-NLS2")
  ...
  const renderEarliestMoveDate = () => (
    <div className="space-y-6">
      ...
      {earliestMoveDate && isWithin60Days(earliestMoveDate) && flags.flag10 && (
        <Alert className="bg-green-50 border-green-200">
          <CheckCircle2 className="h-4 w-4 text-green-500" />
          <AlertDescription className="text-green-700">
            You're willing to move within 60 days, which improves your eligibility. Flag 10 has been removed.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="font-medium">Multiple Occupants (NLS2)</div>
    ...
    <div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium">Pre-Qualification (GC3-NLS2 Non-Applicant)</span>
