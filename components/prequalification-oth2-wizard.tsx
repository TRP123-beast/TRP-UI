...
export function PrequalificationOTH2Wizard() {
  // State
  ...
  // Group members - multiple occupants for OTH2
  ...
  const [flags, setFlags] = useState({
    ...
    flag23: false, // User type changed from OTH2 to CSG2
  })
  ...
  const determineEmploymentTypeCode = () => {
    ...
    if (hasRetired && !hasFullTime && !hasPartTime && !hasSelfEmployed && !hasUnemployed) return "ET_CSG19_Retired"

    if (hasUnemployed) return "ET_CSG20_E:UE"

    if (hasFullTime && !hasPartTime && !hasSelfEmployed) return "ET_CSG21_E:F/T"

    if (!hasFullTime && hasPartTime && !hasSelfEmployed) return "ET_CSG22_E:P/T"

    if (!hasFullTime && !hasPartTime && hasSelfEmployed) return "ET_CSG23_E:SE"

    if (hasFullTime && hasPartTime && !hasSelfEmployed) return "ET_CSG24_E:F/T,E:P/T"

    if (hasFullTime && !hasPartTime && hasSelfEmployed) return "ET_CSG25_E:F/T,E:SE"

    if (!hasFullTime && hasPartTime && hasSelfEmployed) return "ET_CSG26_E:P/T,SE"

    if (hasFullTime && hasPartTime && hasSelfEmployed) return "ET_CSG27_E:F/T,E:P/T,E:SE"
  ...
  // Convert from OTH2 to CSG2
  const convertFromOTH2ToCSG2 = () => {
    console.log("[WORKFLOW] Converting user type from OTH2 to CSG2")
    ...
    <div className="bg-white rounded-lg shadow-sm border p-6">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium">Pre-Qualification (OTH2 Workflow)</span>
